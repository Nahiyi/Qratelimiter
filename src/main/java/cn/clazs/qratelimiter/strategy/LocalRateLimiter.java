...
public class LocalRateLimiter implements RateLimiter {
     ...
     */
    public LocalRateLimiter(int capacity, int freq, long interval) {
        // 参数合法性校验
        if (capacity <= 0) {
    ...
    public LocalRateLimiter(int freq, long interval) {
        this(freq, freq, interval); // capacity 默认为 freq (不推荐，建议由Registry计算最佳capacity)
    }
